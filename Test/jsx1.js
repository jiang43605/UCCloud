function isShowOkNew(e) {
    $("#" + e + "Msg").show(),
    $("#" + e + "Msg").removeClass(e + "-msg-waring"),
    $("#" + e + "Msg").removeClass(e + "-msg-del"),
    $("#" + e + "Msg").addClass(e + "-msg-ok"),
    clearError("message"),
    $("#" + e + "Msg").unbind("click")
}
function isShowFailNew(e, a) {
    $("#" + e + "Msg").hasClass(e + "-msg-del") || $("#" + e + "Msg").unbind("click"),
    clearError("message"),
    $("#" + e + "Msg").show(),
    $("#" + e + "Msg").removeClass(e + "-msg-ok"),
    $("#" + e + "Msg").removeClass(e + "-msg-del"),
    $("#" + e + "Msg").addClass(e + "-msg-waring");
    var r = "<span class='icon-why'></span>" + a;
    $("#dMessageError").html(r)
}
function getPasswordType() {
    return $("#isShowPass").hasClass("hide-pass") ? "password" : "passwordShow"
}
function changeEye() {
    var e = $("#passwordMsg").attr("class"),
    a = $("#passwordShowMsg").attr("class");
    if ($("#isShowPass").hasClass("hide-pass")) {
        $("#isShowPass").removeClass("hide-pass"),
        $("#isShowPass").addClass("show-pass"),
        document.getElementById("passwordShow").value = $("#password").val(),
        $("#password").hide(),
        $("#passwordShow").show();
        var r = e.replace("password", "passwordShow");
        $("#passwordShowMsg").removeClass(a),
        $("#passwordShowMsg").addClass(r)
    } else {
        $("#isShowPass").removeClass("show-pass"),
        $("#isShowPass").addClass("hide-pass"),
        document.getElementById("password").value = $("#passwordShow").val(),
        $("#password").show(),
        $("#passwordShow").hide();
        var r = e.replace("passwordShow", "password");
        $("#passwordMsg").removeClass(e),
        $("#passwordMsg").addClass(r)
    }
}
function getCaptchaNew(e, a) {
    var r = (new Date).getTime(),
    i = "undefined" == typeof $("#display").val() ? "mobile" : $("#display").val();
    $.ajax({
        type: "GET",
        dataType: "json",
        url: e + "getCaptcha?t=" + (new Date).getTime(),
        async: !1,
        data: {
            t: r,
            display: i
        ...
        success: function (e) {
            a || (clearError("message"), $("#captchaValMsg").removeClass("captchaVal-msg-waring"), $("#captchaValMsg").addClass("captchaVal-msg-del"), $("#captchaValMsg").hide()),
            document.getElementById("captchaVal").value = "",
            captchaFlag = !1;
            var r = "data:image/png;base64," + e.data.captchaImage;
            $("#captchaId").val(e.data.captchaId),
            $("#captchaImg").find("img").attr("src", r)
        }
    })
}
function validSpecialCharsNew(e, a) {
    for (var r = "~!@#$%^&*()_+|`-={}[]:\";'<>?,./",
    i = $("#" + e).val(), t = 0; t < i.length; t++) {
        var s = i.charAt(t);
        if (-1 == r.indexOf(s) && !/\w/i.test(s)) return isShowFailNew(e, a),
        !1
    }
    return !0
}
function validIsNumberNew(e, a) {
    var r = new RegExp("^[0-9]*$"),
    i = $.trim($("#" + e).val());
    return r.test(i) ? !0 : (isShowFailNew(e, a), !1)
}
function validNameFormatNew(e) {
    var a = new RegExp("^[0-9]*$"),
    r = new RegExp("^[-_]*$"),
    i = new RegExp("^[-\\w]*$"),
    t = $.trim($("#" + e).val());
    return a.test(t) ? (isShowFailNew(e, error.name.number), !1) : r.test(t) ? (isShowFailNew(e, error.name.specialchars), !1) : i.test(t) ? !0 : (isShowFailNew(e, error.name.character), !1)
}
function validEmptyNew(e, a) {
    var r = $.trim($("#" + e).val()),
    i = $("input[id ='" + e + "']").attr("placeholder");
    return "" == r || r == i ? (isShowFailNew(e, a), !1) : !0
}
function validLengthNew(e, a, r, i) {
    var t = $("#" + e).val().length;
    return a > t || t > r ? (isShowFailNew(e, i), !1) : !0
}
function validIsMobileNew(e, a) {
    var r = new RegExp("^1[0-9]{10}$"),
    i = $("#" + e).val();
    return r.test(i) ? !0 : (isShowFailNew(e, a), !1)
}
function validEmailFormatNew(e, a) {
    var r = $("#" + e).val(),
    i = /^([\w]+([\w-\.+]*[\w-]+)?)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/i;
    return i.test(r) && -1 != r.indexOf(".") ? !0 : (isShowFailNew(e, a), !1)
}
function validPwdNew(e) {
    return "undefined" == typeof e && (e = "password"),
    "undefined" != typeof $("#" + e).val() && "" != $.trim($("#" + e).val()) ? validLengthNew(e, 6, 20, error.password.length) && validSpecialCharsNew(e, error.password.character) ? (isShowOkNew(e), !0) : !1 : void 0
}
function validCaptchaNew(e) {
    return "" != $.trim($("#captchaVal").val()) ? checkCaptherNew(e) ? (captchaFlag = !0, !0) : !1 : void 0
}
function checkCaptherNew(e) {
    var a = $.trim($("#client_id").val()),
    r = $.trim($("#captchaId").val()),
    i = $.trim($("#captchaVal").val()),
    t = "undefined" == typeof $("#display").val() ? "mobile" : $("#display").val(),
    s = !1;
    return $.ajax({
        type: "GET",
        url: e + "ajax/checkCaptcha",
        dataType: "json",
        data: {
            captchaId: r,
            uc_param_str: "nieisivefrpfbimilaprligiwiut",
            captchaVal: i,
            display: t,
            client_id: a
        },
        async: !1,
        success: function (a) {
            null == a || "2000000" == a.status ? (isShowOkNew("captchaVal"), s = !0) : (isShowFailNew("captchaVal", error.captcha.character), getCaptchaNew(e, !0), s = !1)
        }
    }),
    s
}
function vaildRegisterNameNew(e, a, r) {
    if ("" != $.trim($("#registerName").val())) {
        var i = !1;
        return a ? i = validNameNew(e) : (registerNameType = "undefined" == typeof r || "" == r ? vaildRegisterNameType() : r, "mobile" == registerNameType ? i = validMobileNew(e) : "email" == registerNameType ? i = validEmailNew(e) : (isShowFailNew("registerName", error.registerName.character), i = !1)),
        registerNameFlag = i,
        i
    }
}
function vaildRegisterNameType() {
    var e = $.trim($("#registerName").val()),
    a = new RegExp("^1[0-9]{10}$");
    11 == e.length && a.test(e) && (registerNameType = "mobile");
    var r = /^([\w]+([\w-\.+]*[\w-]+)?)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/i;
    return r.test(e) && e.indexOf(".") > 0 && (registerNameType = "email"),
    registerNameType
}
function validMobileNew(e) {
    return validIsMobileNew("registerName", error.mobile.character) ? checkMobileNew(e) : !1
}
function checkMobileNew(e) {
    var a = $.trim($("#client_id").val()),
    r = $("#registerName").val(),
    i = (new Date).getTime(),
    t = "undefined" == typeof $("#display").val() ? "" : $("#display").val(),
    s = "undefined" == typeof $("#browser_type").val() ? "" : $("#browser_type").val(),
    l = !1;
    return $.ajax({
        type: "POST",
        url: e + "ajax/checkMobile",
        dataType: "json",
        data: {
            registerName: r,
            uc_param_str: "nieisivefrpfbimilaprligiwiut",
        ...
        async: !1,
        success: function (e) {
            null == e || "2000000" == e.status || "40004" == e.status ? (isShowOkNew("registerName"), l = !0) : (isShowFailNew("registerName", e.message), l = !1)
        }
    }),
    l
}
function validAgreementNew() {
    return clearError("agreement"),
    clearError("message"),
    "undefined" == typeof $("input[name='agreement']:checked").val() ? (isShowFailNew("agreement", error.agreement.isChecked), !1) : !0
}
function validEmailNew(e) {
    return validEmailFormatNew("registerName", error.email.wrongformat) ? checkEmailNew(e) : !1
}
function checkEmailNew(e) {
    var a = $.trim($("#client_id").val()),
    r = $("#registerName").val(),
    i = (new Date).getTime(),
    t = "undefined" == typeof $("#display").val() ? "" : $("#display").val(),
...
function validNameNew(e) {
    return "" != $.trim($("#registerName").val()) ? validLengthNew("registerName", 4, 16, error.name.length) && validNameFormatNew("registerName") ? checkNameNew(e) : !1 : void 0
}
function checkNameNew(e) {
    var a = $.trim($("#client_id").val()),
    r = $("#registerName").val(),
    i = (new Date).getTime(),
    t = "undefined" == typeof $("#display").val() ? "" : $("#display").val(),
...
function vaildRegisterNew(e, a, r) {
    var i = !0;
    return validEmptyNew("registerName", error.registerName.empty) && validEmptyNew(getPasswordType(), error.password.empty) && ("undefined" == typeof $("#captchaId").val() || "" == $("#captchaId").val() || captchaFlag || validEmptyNew("captchaVal", error.captcha.empty)) ? (registerNameFlag || vaildRegisterNameNew(e, a, r)) && validPwdNew(getPasswordType()) && ("undefined" == typeof $("#captchaId").val() || "" == $("#captchaId").val() || captchaFlag || validCaptchaNew(e)) && validAgreementNew() ? i : i = !1 : !1
}
function submitRegFormNew(e, a, r) {
    if (!vaildRegisterNew(e, a, r)) return unloading(error.registerbutton.unloading),
    !1;
    var i = document.getElementById("dRegisterForm"),
    t = i.action,
    s = ($("#submit_btn").attr("casBaseUriWithHttps"), $("#submit_btn").attr("ucParamStr")); ("undefined" == typeof r || "" == r) && ("mobile" == registerNameType ? i.action = t + "sendMobileVcode?register_type=mobile&new_mobile_register=1&uc_param_str=" + s : "email" == registerNameType && (i.action = t + "sendEmailForRegister?register_type=email&uc_param_str=" + s)),
    "passwordShow" == getPasswordType() && (document.getElementById("password").value = $("#passwordShow").val()),
    loading(error.registerbutton.loading),
    i.submit(),
    clearAllError(),
    unloading(error.registerbutton.unloading)
}
function validMobileCodeNew() {
    var e = !0;
    return e = validEmptyNew("mobileCode", "è¯·è¾“å…¥çŸ­ä¿¡éªŒè¯ç "),
    e ? e : (e = !1, !1)
}
function submitMobileFormNew() {
    var e = document.getElementById("dRegisterMobileForm");
    e.submit()
}
function validLoginNameNew() {
    return "" != $.trim($("#loginName").val()) ? (loginNameFlag = !0, isShowOkNew("loginName"), !0) : void 0
}
function validLoginNew(e) {
    var a = !0;
    return validEmptyNew("loginName", error.login.empty) && validEmptyNew("password", error.password.empty) && ("undefined" == typeof $("#captchaId").val() || "" == $("#captchaId").val() || validEmptyNew("captchaVal", error.captcha.empty)) ? (loginNameFlag || validLoginNameNew(e)) && validPwdNew("password") && ("undefined" == typeof $("#captchaId").val() || "" == $("#captchaId").val() || captchaFlag || validCaptchaNew(e)) ? a : a = !1 : !1
}
function submitFormNew(e) {
    var a = $("#dLoginForm");
    return null != $("#commitloginId") && $("#commitloginId").val((new Date).getTime()),
    validLoginNew(e) ? (loading(error.loginbutton.loading), a.submit(), clearAllError(), void unloading(error.loginbutton.unloading)) : (unloading(error.loginbutton.unloading), !1)
}
function upSmsForContinueRegister() {
    var e = $("#dLoginForm");
    validPwdNew("password") && e.submit()
}
var captchaFlag = !1,
registerNameFlag = !1,
registerNameType = "",
...
var Zepto = function() {
    ...
    function c(t, n) {
        return "number" != typeof n || O[s(t)] ? n: n + "px"
    }
    function l(t) {
        var n, e;
        return j[t] || (n = T.createElement(t), T.body.appendChild(n), e = N(n, "").getPropertyValue("display"), n.parentNode.removeChild(n), "none" == e && (e = "block"), j[t] = e),
        j[t]
    }
    function f(t, n) {
        return n === m ? v(t) : v(t).filter(n)
    }
    function h(n, e, i, r) {
    ...
    function p(t, n, e) {
        ...
        i ? i.insertBefore(e, t ? 1 == t ? i.firstChild: 2 == t ? n: null: n.nextSibling) : v(e).remove()
    }
    function d(t, n) {
        n(t);
    ...
    D.fragment = function(t, n) {
        n === m && (n = C.test(t) && RegExp.$1),
        n in Z || (n = "*");
        var e = Z[n];
        return e.innerHTML = "" + t,
        v.each(E.call(e.childNodes),
        function() {
            e.removeChild(this)
        })
    },
    ...
    D.init = function(n, r) {
        if (!n) return D.Z();
        if (t(n)) return v(T).ready(n);
        if (D.isZ(n)) return n;
        var a;
        if (i(n)) a = o(n);
        else if (e(n)) a = [v.extend({},
        n)],
        n = null;
        else if (P.indexOf(n.nodeType) >= 0 || n === window) a = [n],
        n = null;
        else if (C.test(n)) a = D.fragment(n.trim(), RegExp.$1),
        n = null;
        else {
            if (r !== m) return v(r).find(n);
            a = D.qsa(T, n)
        }
        return D.Z(a, n)
    },
    ...
    v.fn = {
        ...
        ready: function(t) {
            return R.test(T.readyState) ? t(v) : T.addEventListener("DOMContentLoaded",
            function() {
                t(v)
            },
            !1),
        ...
        not: function(n) {
            ...
            if (t(n) && n.call !== m) this.each(function(t) {
                ...
                var i = "string" == typeof n ? this.filter(n) : r(n) && t(n.item) ? E.call(n) : v(n);
        ...
        parents: function(t) {
            for (var n = [], e = this; e.length > 0;) e = v.map(e,
            function(t) {
        ...
        show: function() {
            return this.each(function() {
                ...
                "none" == N(this, "").getPropertyValue("display") && (this.style.display = l(this.nodeName))
            })
        },
        ...
        html: function(t) {
            return t === m ? this.length > 0 ? this[0].innerHTML: null: this.each(function(n) {
                ...
                v(this).empty().append(h(this, t, n, e))
            })
        },
        text: function(t) {
        ...
        attr: function(t, e) {
            ...
            return "string" == typeof t && e === m ? 0 == this.length || 1 !== this[0].nodeType ? m: "value" == t && "INPUT" == this[0].nodeName ? this.val() : !(i = this[0].getAttribute(t)) && t in this[0] ? this[0][t] : i: this.each(function(i) {
                if (1 === this.nodeType) if (n(t)) for (g in t) this.setAttribute(g, t[g]);
        ...
        addClass: function(t) {
            return this.each(function(n) {
                ...
                this),
                y.length && (this.className += (e ? " ": "") + y.join(" "))
            })
        },
        ...
        toggleClass: function(t, n) {
            return this.each(function(e) {
                var i = h(this, t, e, this.className); (n === m ? !v(this).hasClass(i) : n) ? v(this).addClass(i) : v(this).removeClass(i)
            })
        }
    },
        ...
        v.fn[t] = function() {
            ...
            return this.each(function(n, a) {
                for (var s = 0; s < t.length; s++) {
                    ...
                    r && i - 1 > n && (u = u.cloneNode(!0)),
                    p(e, a, u)
                }
            })
        },
        v.fn[e % 2 ? t + "To": "insert" + (e ? "Before": "After")] = function(n) {
            return v(n)[t](this),
            this
        }
    }),
...
function(t) {
    function n(t) {
        ...
        c = t.match(/Kindle\/([\d.]+)/),
        l = t.match(/Silk\/([\d._]+)/),
        f = t.match(/(BlackBerry).*Version\/([\d.]+)/); (e.webkit = !!i) && (e.version = i[1]),
...
function(t, n) {
    ...
    t.each(a,
    function(t, i) {
        return u.style[t + "TransitionProperty"] !== n ? (o = "-" + e(t) + "-", r = i, !1) : void 0
    }),
    ...
    t.fn.anim = function(e, i, r, a) {
        ...
        if (i === n && (i = .4), t.fx.off && (i = 0), "string" == typeof e) h[o + "animation-name"] = e,
        h[o + "animation-duration"] = i + "s",
        d = t.fx.animationEnd;
        else {
            for (u in e) c.test(u) ? (s || (s = []), s.push(u + "(" + e[u] + ")")) : h[u] = e[u];
            s && (h[o + "transform"] = s.join(" ")),
            !t.fx.off && "object" == typeof e && (h[o + "transition-property"] = Object.keys(e).join(", "), h[o + "transition-duration"] = i + "s", h[o + "transition-timing-function"] = r || "linear")
        }
        return f = function(n) {
...
function(t) {
    ...
    function h(n) {
        v(n.data) && (n.data = t.param(n.data)),
    ...
    function p(n, e, i, r) {
        ...
        function(e, a) {
            r && (e = i ? r: r + "[" + (o ? "": e) + "]"),
            !r && o ? n.add(a.name, a.value) : (i ? t.isArray(a) : v(a)) ? p(n, a, i, e) : n.add(e, a)
        })
    }
    var d, m, g = 0,
    v = t.isObject,
    ...
    t.ajax = function(n) {
        ...
        g && (v.Accept = g, g.indexOf(",") > -1 && (g = g.split(",", 2)[0]), w.overrideMimeType && w.overrideMimeType(g)),
        (e.contentType || e.data && "GET" != e.type.toUpperCase()) && (v["Content-Type"] = e.contentType || "application/x-www-form-urlencoded"),
        e.headers = t.extend(v, e.headers || {}),
        w.onreadystatechange = function() {
            if (4 == w.readyState) {
                ...
                if (w.status >= 200 && w.status < 300 || 304 == w.status || 0 == w.status && "file:" == y) {
                    ...
                    t = w.responseText;
                    try {
                        "script" == r ? (1, eval)(t) : "xml" == r ? t = w.responseXML: "json" == r && (t = j.test(t) ? null: JSON.parse(t))
                    } catch(i) {
                        n = i
                    }
                    n ? s(n, "parsererror", w, e) : a(t, w, e)
                } else s(null, "error", w, e)
            }
        };
    ...
    t.fn.load = function(n, e) {
        ...
        t.get(n,
        function(n) {
            r.html(i ? t(y.createElement("div")).html(n.replace(w, "")).find(i).html() : n),
            e && e.call(r)
        }),
        this
    };
